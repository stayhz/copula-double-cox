...
simu_dep2 <- function(vv, n, p, q, truepara, copula_name, dist, numseed0, pA) {
  start_time <- Sys.time()
  ...
  # --- Retry mechanism in case optimization fails ---
  retry_count <- 0
  success <- FALSE
  ...
  while (!success && retry_count <= 5) {
    try({
      ...
      # --- Step 2: Generate dependent data ---
      gendata <- switch(
        dist,
      ...
      # --- Step 4: Fit independent double-Cox model (misspecified) ---
